...
def test_dictionary_units():
    ...
    for key in _original_dictionary:
        # ignore helper entries (reverse maps, plural tables, etc.)
        if '_' in key:
            continue
...
def test_canonical_and_all_units():
    """Verify that alias canonicalization and the _all_units helper behave correctly."""
    from unyts.dictionaries import canonical_name, _all_units
    # simple alias -> canonical
    assert canonical_name('ml') == 'millilitre'
    # ambiguous alias should be left alone
    assert canonical_name('oz') == 'oz'
    # _all_units must contain both canonical names and common aliases
    allu = _all_units()
    assert 'millilitre' in allu
    assert 'ml' in allu
    assert 'Coulomb' in allu
