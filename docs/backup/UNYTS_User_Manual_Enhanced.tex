...
\begin{tikzpicture}[
  node distance=2.5cm,
  every node/.style={circle, draw, minimum size=1cm, font=\sffamily},
  every edge/.style={draw, -Stealth, thick}
]
  \node (m) {meter};
  \node (km) [above right of=m] {km};
  \node (cm) [below right of=m] {cm};
  \node (yd) [right of=m, xshift=1cm] {yard};
  \node (ft) [right of=yd] {foot};
  
  \path[every edge/.style={draw, -Stealth, thick, blue!60}]
    (m) edge node[above, font=\tiny, fill=white] {$\times 0.001$} (km)
    (m) edge node[below, font=\tiny, fill=white] {$\times 100$} (cm)
    (m) edge node[above, font=\tiny, fill=white] {$\div 0.9144$} (yd)
    (yd) edge node[above, font=\tiny, fill=white] {$\times 3$} (ft);
    
  \path[every edge/.style={draw, -Stealth, thick, red!60}]
    (km) edge[bend left=20] node[above, font=\tiny, fill=white] {$\times 1000$} (m)
    (cm) edge[bend left=20] node[below, font=\tiny, fill=white] {$\div 100$} (m)
    (yd) edge[bend left=20] node[below, font=\tiny, fill=white] {$\times 0.9144$} (m)
    (ft) edge[bend left=20] node[below, font=\tiny, fill=white] {$\div 3$} (yd);
\end{tikzpicture}
\caption{Simple directed graph showing length unit conversions. Blue edges represent forward conversions, red edges represent reverse conversions.}
...
\begin{tikzpicture}[
  ...
  every node/.style={circle, draw, font=\small\sffamily, minimum size=0.8cm},
  ...
  \draw[dashed, thick, red!70, rounded corners] 
    ($(start) + (-0.5, 0.3)$) -- ($(yd) + (0.5, 0.3)$) -- 
    ($(ft) + (0.5, 0.3)$) -- ($(in) + (0.5, -0.3)$) --
    ($(in) + (-0.5, -0.3)$) -- ($(ft) + (-0.5, -0.3)$) --
    ($(yd) + (-0.5, -0.3)$) -- ($(start) + (-0.5, 0.3)$);
  
  \node[above right of=in, xshift=1cm, draw=none, font=\footnotesize\sffamily, align=left] 
    {\textbf{Start:} m \\ \textbf{Goal:} in \\ \textbf{Layers:} 1, 2, 3 \\ \textbf{Path:} m→yd→ft→in};
\end{tikzpicture}
\caption{BFS exploration tree. Yellow nodes explored before red goal node (in). Green node is start. Dashed line shows shortest path found.}
\label{fig:bfs_tree}
\end{figure}
...
\begin{tikzpicture}[
  ...
  \node[right of=in, xshift=1.5cm, draw=none, font=\footnotesize\sffamily, align=left] 
    {\textbf{Order:} m→km (dead end) \\ m→cm→mm (dead end) \\ m→yd→ft→in (found!)};
\end{tikzpicture}
\caption{DFS exploration. Thick red arrows show the path DFS follows. Yellow nodes explored before finding goal.}
\label{fig:dfs_tree}
\end{figure}
...
\begin{tikzpicture}[
  node distance=1.8cm,
  every node/.style={rectangle, rounded corners, draw, minimum width=1.2cm, minimum height=0.7cm, font=\small\sffamily},
  every edge/.style={draw, -Stealth}
]
  \node[fill=blue!20] (m) {meter};
  \node[above left of=m] (km) {km};
  \node[left of=m, xshift=-0.5cm] (hm) {hm};
  \node[below left of=m] (dam) {dam};
  \node[above right of=m] (dm) {dm};
  \node[right of=m, xshift=0.5cm] (cm) {cm};
  \node[below right of=m] (mm) {mm};
  
  \path[every edge/.style={<->, blue!60, thick}]
    (m) edge node[above left, font=\tiny] {×0.001} (km)
    (m) edge node[above, font=\tiny] {×0.01} (hm)
    (m) edge node[below left, font=\tiny] {×0.1} (dam)
    (m) edge node[above right, font=\tiny] {×10} (dm)
    (m) edge node[above, font=\tiny] {×100} (cm)
    (m) edge node[below right, font=\tiny] {×1000} (mm);
\end{tikzpicture}
\caption{SI prefix network for length. All conversions route through the base unit (meter).}
\label{fig:si_length}
\end{figure}
...
\begin{tikzpicture}[
  node distance=2.2cm,
  every node/.style={rectangle, rounded corners, draw, minimum width=1.2cm, minimum height=0.7cm, font=\small\sffamily},
  every edge/.style={draw, <->, thick, orange!70}
]
  \node (in) {inch};
  \node[right of=in] (ft) {foot};
  \node[right of=ft] (yd) {yard};
  \node[right of=yd] (ch) {chain};
  \node[right of=ch] (fur) {furlong};
  \node[right of=fur] (mi) {mile};
  
  \path
    (in) edge node[above, font=\tiny] {×12} (ft)
    (ft) edge node[above, font=\tiny] {×3} (yd)
    (yd) edge node[above, font=\tiny] {×22} (ch)
    (ch) edge node[above, font=\tiny] {×10} (fur)
    (fur) edge node[above, font=\tiny] {×8} (mi);
    
  \node[below of=in, yshift=0.5cm] (th) {thou};
  \node[below of=ft, xshift=-1.1cm, yshift=0.5cm] (tenth) {tenth};
  
  \path[<->, dashed, orange!50]
    (in) edge node[right, font=\tiny] {×1000} (th)
    (in) edge node[right, font=\tiny] {×10} (tenth);
\end{tikzpicture}
\caption{Imperial length chain. Units connected sequentially by their definitions.}
...
\begin{tikzpicture}[
  ...
  % SI side
  \node[fill=blue!20] (m) {meter};
  \node[above of=m, yshift=-0.8cm] (km) {km};
  \node[below of=m, yshift=0.8cm] (cm) {cm};
  
  % Imperial side
  \node[fill=orange!20, right of=m, xshift=2cm] (yd) {yard};
  \node[right of=yd, xshift=0.5cm] (ft) {foot};
  \node[right of=ft, xshift=0.5cm] (in) {inch};
  ...
  \path[<->, orange!70, thick]
    (yd) edge node[above, font=\tiny] {×3} (ft)
    (ft) edge node[above, font=\tiny] {×12} (in);
  
  \path[<->, very thick, red!70]
    (m) edge node[above, font=\footnotesize\bfseries] {Bridge} (yd);
\end{tikzpicture}
\caption{The SI-Imperial bridge at meter↔yard. All conversions between systems must route through this connection.}
...
\begin{tikzpicture}[
  node distance=1.5cm,
  every node/.style={rectangle, rounded corners, draw, minimum width=1cm, minimum height=0.6cm, font=\scriptsize\sffamily},
  standard/.style={draw, <->, thick, gray!50},
  highlight/.style={draw, <->, ultra thick, red!70}
]
  % SI system
  \node[fill=blue!10] (m) {meter};
  \node[above left of=m, yshift=-0.3cm] (km) {km};
  \node[below left of=m, yshift=0.3cm] (mm) {mm};
  \node[left of=m, yshift=0.5cm, fill=yellow!30] (cm) {cm};
  
  % Imperial system
  \node[fill=orange!10, right of=m, xshift=2.5cm] (yd) {yard};
  \node[right of=yd, xshift=0.5cm] (ft) {foot};
  \node[right of=ft, xshift=0.5cm, fill=yellow!30] (in) {inch};
  \node[above of=ft, yshift=-0.5cm] (ch) {chain};
  \node[right of=ch, xshift=0.5cm] (mi) {mile};
  
  % Standard connections (gray)
  \path[standard]
    (m) edge (km)
    (m) edge (mm)
    (yd) edge (ch)
  ...
  % Highlighted path (red)
  \path[highlight]
    (cm) edge node[above, font=\tiny, fill=white] {÷100} (m)
    (m) edge node[above, font=\tiny, fill=white] {÷0.9144} (yd)
    (yd) edge node[above, font=\tiny, fill=white] {×3} (ft)
    (ft) edge node[above, font=\tiny, fill=white] {×12} (in);
    
  \node[below of=in, yshift=1cm, draw=none, font=\footnotesize\sffamily, align=center] 
    {\textbf{Path: cm → m → yd → ft → in} \\
     100 cm = $100 \div 100 \times (1/0.9144) \times 3 \times 12$ \\
     = 39.37 inches};
\end{tikzpicture}
\caption{Complete length network showing highlighted conversion path from centimeter to inch (yellow nodes). Red thick arrows trace the conversion route.}
...
\begin{tikzpicture}[
  ...
  every node/.style={rectangle, rounded corners, draw, font=\small\sffamily, minimum width=1.2cm},
...
\end{tikzpicture}
\caption{Time network hierarchical structure. Yellow nodes (week, month) branch from day.}
...
\begin{tikzpicture}[
  node distance=3.5cm,
  every node/.style={circle, draw, minimum size=1.8cm, font=\small\sffamily, align=center},
  every edge/.style={draw, <->, thick}
]
  \node[fill=blue!20] (C) {Celsius};
  \node[fill=blue!10, right of=C] (K) {Kelvin};
  \node[fill=red!20, below of=C] (F) {Fahrenheit};
  \node[fill=red!10, right of=F] (R) {Rankine};
  
  \path[blue!70]
    (C) edge node[above, font=\tiny, fill=white] {$K = C + 273.15$} (K);
    
  \path[red!70]
    (F) edge node[below, font=\tiny, fill=white] {$R = F + 459.67$} (R);
    
  \path[purple!70]
    (C) edge node[left, font=\tiny, fill=white, align=center] {$F = C \times 9/5 + 32$} (F);
    
  \path[green!60!black]
    (K) edge node[right, font=\tiny, fill=white, align=center] {$R = K \times 9/5$} (R);
\end{tikzpicture}
\caption{Temperature network showing all bidirectional conversions. Each edge encodes both offset and scale transformations.}
