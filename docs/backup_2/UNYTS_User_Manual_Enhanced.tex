...
\begin{tikzpicture}[
  ...
  \node[below of=start, yshift=-4.5cm, draw=black, very thick, fill=white, rounded corners, inner sep=0.4cm, font=\small\sffamily, align=center] 
    {\textbf{Start:} m \\[0.1cm] \textbf{Goal:} in \\[0.1cm] \textbf{Layers:} 1, 2, 3 \\[0.1cm] \textbf{Path:} m {$\to$} yd {$\to$} ft {$\to$} in};
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  \node[below of=start, yshift=-4.5cm, draw=black, very thick, fill=white, rounded corners, inner sep=0.4cm, font=\small\sffamily, align=center] 
    {\textbf{Exploration Order:} \\[0.1cm] m{$\to$}km (dead end) \\[0.1cm] m{$\to$}cm{$\to$}mm (dead end) \\[0.1cm] m{$\to$}yd{$\to$}ft{$\to$}in (found!)};
\end{tikzpicture}
...
\begin{tikzpicture}[
  node distance=3.5cm,
  unit/.style={circle, draw, minimum size=1.8cm, font=\small\sffamily, align=center},
  formula/.style={rectangle, rounded corners, draw, very thick, minimum width=1.2cm, minimum height=0.5cm, font=\tiny, inner sep=0.2cm},
  every edge/.style={draw, <->, thick}
]
  \node[unit, fill=blue!20] (C) {Celsius};
  \node[unit, fill=blue!10, right of=C] (K) {Kelvin};
  \node[unit, fill=red!20, below of=C] (F) {Fahrenheit};
  \node[unit, fill=red!10, right of=F] (R) {Rankine};
  
  \path[blue!70]
    (C) edge node[formula, draw=blue!70, fill=blue!5, above] {$K = C + 273.15$} (K);
  \path[red!70]
    (F) edge node[formula, draw=red!70, fill=red!5, below] {$R = F + 459.67$} (R);
  \path[purple!70]
    (C) edge node[formula, draw=purple!70, fill=purple!5, left, align=center] {$F = C \times 9/5 + 32$} (F);
  \path[green!60!black]
    (K) edge node[formula, draw=green!60!black, fill=green!15, right, align=center] {$R = K \times 9/5$} (R);
\end{tikzpicture}
